...
public class ProxyPool {
	...
	private String proxyFilePath = "/data/crawler/lastUse.proxy";
	...
	private void setFilePath() {
		...
		String path = tmpDir + FilePersistentBase.PATH_SEPERATOR + "crawler" + FilePersistentBase.PATH_SEPERATOR
				+ "lastUse.proxy";
		if (tmpDir != null && new File(tmpDir).isDirectory()) {
			fBase.setPath(tmpDir + FilePersistentBase.PATH_SEPERATOR + "crawler");
	...
	public void returnProxy(HttpHost host, int statusCode) {
		...
		if (p.getFailedNum() > 20) {
			...
			logger.error("remove proxy >>>> " + host + ">>>>" + p.getFailedType() + " >>>> remain proxy >>>> "
					+ proxyQueue.size());
			return;
		}
